...
public class ItemEnquiryServiceImpl implements ItemEnquiryService {
    ...
    public AvailableStockDto getAvailableStockList(String productCategory, String itemDescription, String brand,
        ...
        for (StockSummaryEntity stock : stockList) {
            ...
            // Use bundle-level quantities when bundles exist (matches inventory-wise page logic)
            ...
            if (bundles != null && !bundles.isEmpty()) {
                for (StockSummaryBundleEntity bundle : bundles) {
                    if (bundle.getWeightmentQuantityKg() != null) {
                        totalKg = totalKg.add(bundle.getWeightmentQuantityKg());
                    }
                    if (bundle.getWeightmentQuantityNo() != null) {
                        totalNo += bundle.getWeightmentQuantityNo();
                    }
                }
            } else {
                // Fallback to stock entity quantities (opening stock entries without bundles)
