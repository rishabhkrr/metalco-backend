...
//    }
    ...
    @Override
    public Map<String, Object> findBestEligibleBin(String itemCategory, Double bundleNetWeight, String store, String unitName, Double additionalAllocatedWeight) {

        // Normalize additionalAllocatedWeight
        if (additionalAllocatedWeight == null) additionalAllocatedWeight = 0.0;
        ...
        log.info("│ Required capacity (bundleNetWeight): {} kg, additionalAllocated: {} kg", bundleNetWeight, additionalAllocatedWeight);

        List<RackBinMasterEntity> capacityFilteredBins = filterByCapacity(categoryFilteredBins, bundleNetWeight, additionalAllocatedWeight);
        ...
        if (capacityFilteredBins.isEmpty() && !usedFallbackCategory) {
            ...
            if (!fallbackCategoryBins.isEmpty()) {
                capacityFilteredBins = filterByCapacity(fallbackCategoryBins, bundleNetWeight, additionalAllocatedWeight);
     ...
     * bundleNetWeight
     */
    private List<RackBinMasterEntity> filterByCapacity(List<RackBinMasterEntity> bins, Double bundleNetWeight, Double additionalAllocatedWeight) {
        return bins.stream()
                .filter(bin -> {
                    Double availableSpace = calculateAvailableSpace(bin) - additionalAllocatedWeight;
                    boolean hasCapacity = availableSpace >= bundleNetWeight;
                    log.debug("   Bin ID={}: capacity={}, current={}, additional={}, available={}, required={}, hasCapacity={}",
                            bin.getId(), bin.getBinCapacity(), bin.getCurrentStorage(),
                            additionalAllocatedWeight, availableSpace, bundleNetWeight, hasCapacity);
                    return hasCapacity;
                })
                .toList();
    }
