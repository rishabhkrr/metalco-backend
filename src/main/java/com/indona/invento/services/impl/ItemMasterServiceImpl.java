...
public class ItemMasterServiceImpl implements ItemMasterService {
    ...
    @Override
    public List<ItemMasterEntity> getAllApprovedItems() {
    ...
    public ItemMasterEntity approveItem(Long id) throws Exception {
        ...
        // Ensure a stock summary entry exists with opening stock values
        // This handles bulk-uploaded items that didn't get a stock summary entry at creation time
        ensureStockSummaryExists(approved);
    ...
    /**
     * Ensure a stock summary entry exists for the given item.
     * If no entry exists and the item has opening stock, create one.
     * If an entry exists with 0 quantities but the item has opening stock, update it.
     */
    private void ensureStockSummaryExists(ItemMasterEntity item) {
        try {
            BigDecimal openingKg = item.getOpeningStockInKgs() != null ? item.getOpeningStockInKgs() : BigDecimal.ZERO;
            BigDecimal openingNos = item.getOpeningStockInNos() != null ? item.getOpeningStockInNos() : BigDecimal.ZERO;

            // Check if a stock summary entry already exists for this item
            String skuDesc = item.getSkuDescription();
            if (skuDesc == null || skuDesc.isBlank()) return;

            // Look for existing stock summary entries matching this item
            java.util.List<com.indona.invento.entities.StockSummaryEntity> existing =
                    stockSummaryService.getAll().stream()
                            .filter(s -> skuDesc.equalsIgnoreCase(s.getItemDescription()))
                            .collect(java.util.stream.Collectors.toList());

            if (existing.isEmpty()) {
                // No stock summary entry exists — create one if opening stock > 0
                if (openingKg.compareTo(BigDecimal.ZERO) > 0 || openingNos.compareTo(BigDecimal.ZERO) > 0) {
                    logger.info("📦 Creating stock summary for approved item: {} (Opening: {} KG, {} NOS)",
                            skuDesc, openingKg, openingNos);
                    ...
                    StockSummaryDto stockDto = StockSummaryDto.builder()
                            ...
                            .quantityNo(openingNos.intValue())
                            .itemPrice(item.getItemPrice() != null ? item.getItemPrice() : BigDecimal.ZERO)
                            .reprintQr(false)
                            .sectionNo(item.getSectionNumber())
                            .build();

                    stockSummaryService.create(stockDto);
                    logger.info("✅ Stock summary created for: {}", skuDesc);
                }
            } else {
                // Entry exists — update opening stock if the entry has 0 qty and item has opening stock
                for (com.indona.invento.entities.StockSummaryEntity entry : existing) {
                    BigDecimal currentKg = entry.getQuantityKg() != null ? entry.getQuantityKg() : BigDecimal.ZERO;
                    int currentNo = entry.getQuantityNo() != null ? entry.getQuantityNo() : 0;

                    if (currentKg.compareTo(BigDecimal.ZERO) == 0 && currentNo == 0
                            && (openingKg.compareTo(BigDecimal.ZERO) > 0 || openingNos.compareTo(BigDecimal.ZERO) > 0)) {
                        logger.info("📦 Updating stock summary ID {} with opening stock: {} KG, {} NOS",
                                entry.getId(), openingKg, openingNos);
                        entry.setQuantityKg(openingKg);
                        entry.setQuantityNo(openingNos.intValue());
                        if (item.getItemPrice() != null) {
                            entry.setItemPrice(item.getItemPrice());
                        }
                        // Save is handled by the service layer
                        stockSummaryService.update(entry.getId(), StockSummaryDto.builder()
                                .unit(entry.getUnit())
                                .store(entry.getStore())
                                .storageArea(entry.getStorageArea())
                                .rackColumnShelfNumber(entry.getRackColumnShelfNumber())
                                .productCategory(entry.getProductCategory())
                                .itemDescription(entry.getItemDescription())
                                .brand(entry.getBrand())
                                .materialType(entry.getMaterialType())
                                ...
                                .quantityNo(openingNos.intValue())
                                .itemPrice(item.getItemPrice() != null ? item.getItemPrice() : BigDecimal.ZERO)
                                .reprintQr(entry.getReprintQr())
                                .sectionNo(entry.getSectionNo())
                                .build());
                        logger.info("✅ Stock summary updated for: {}", skuDesc);
                        break; // Only update the first matching entry
                    }
                }
            }
        } catch (Exception e) {
            logger.warn("⚠️ Failed to ensure stock summary for item {}: {}", item.getSkuDescription(), e.getMessage());
            // Don't fail the approval if stock summary creation fails
        }
    }
    ...
    public List<ItemMasterEntity> getItemsByCategoryAndBrand(String productCategory, String brand) {
        ...
        logger.info("📦 Found {} matching approved items", items.size());
        return items;
    }
    ...
    public Map<String, Object> getDimensionByItemDescription(String itemDescription) {
        ...
        // Find all APPROVED items matching the description (case-insensitive)
        List<ItemMasterEntity> matchingItems = repository.findAll().stream()
                .filter(item -> "APPROVED".equalsIgnoreCase(item.getStatus()) &&
                        item.getSkuDescription() != null &&
