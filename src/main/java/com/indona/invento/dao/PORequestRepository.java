...
public interface PORequestRepository extends JpaRepository<PORequestEntity, Long> {
    ...
    @Query("SELECT p FROM PORequestEntity p WHERE " +
            "(:supplierCode IS NULL OR p.supplierCode = :supplierCode) AND " +
            "(:supplierName IS NULL OR p.supplierName = :supplierName) AND " +
            "(:unitCode IS NULL OR p.unitCode = :unitCode) AND " +
            "p.status = 'PENDING'")
