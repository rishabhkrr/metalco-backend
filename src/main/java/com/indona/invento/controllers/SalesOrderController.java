...
//    }
    ...
    public ResponseEntity<?> getPendingApprovalSalesOrders() {
        try {
            List<SalesOrder> result = salesOrderService.getPendingApprovalSalesOrders();
            return ResponseEntity.ok(result);
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(Map.of("error", e.getMessage()));
        }
    ...
    public ResponseEntity<?> approveSalesOrderOverdue(
        ...
        try {
            ...
            Map<String, Object> response = new HashMap<>();
            response.put("message", "✅ Sales Order approved successfully");
            response.put("soNumber", approved.getSoNumber());
            response.put("status", approved.getStatus());
            response.put("approvalRemarks", approved.getApprovalRemarks());
            return ResponseEntity.ok(response);
        } catch (RuntimeException e) {
            return ResponseEntity.status(404).body(Map.of("error", e.getMessage()));
        }
    ...
    public ResponseEntity<?> rejectSalesOrderOverdue(
        ...
        try {
            ...
            Map<String, Object> response = new HashMap<>();
            response.put("message", "❌ Sales Order rejected");
            response.put("soNumber", rejected.getSoNumber());
            response.put("status", rejected.getStatus());
            response.put("approvalRemarks", rejected.getApprovalRemarks());
            return ResponseEntity.ok(response);
        } catch (RuntimeException e) {
            return ResponseEntity.status(404).body(Map.of("error", e.getMessage()));
        }
