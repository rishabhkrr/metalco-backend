...
public class StockSummaryController {
    ...
    /**
     * Get blocked quantity details for a given item description
     * Returns quotation-level blocked stock details for drill-down popup
     *
     * Usage: GET /api/metalco/stock-summary/blocked-details?itemDescription=...
     */
    @GetMapping("/blocked-details")
    public ResponseEntity<?> getBlockedDetails(@RequestParam String itemDescription) {
        try {
            List<Map<String, Object>> details = service.getBlockedDetails(itemDescription);
            return ResponseEntity.ok(Map.of(
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(Map.of("error", "Failed to fetch blocked details: " + e.getMessage()));
        }
    }

    /**
     * Get SO allotted quantity details for a given item description
     * Returns sales-order-level allocation details for drill-down popup
     *
     * Usage: GET /api/metalco/stock-summary/so-allotted-details?itemDescription=...
     */
    @GetMapping("/so-allotted-details")
    public ResponseEntity<?> getSoAllottedDetails(@RequestParam String itemDescription) {
        try {
            List<Map<String, Object>> details = service.getSoAllottedDetails(itemDescription);
            return ResponseEntity.ok(Map.of(
                    "totalCount", details.size(),
                    "data", details,
                    "itemDescription", itemDescription
