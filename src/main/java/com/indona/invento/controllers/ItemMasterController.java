...
public class ItemMasterController {
    ...
    /**
     * Get only APPROVED items - for use by other modules (Quotation, SO, Stock Summary)
     * Items with status PENDING_APPROVAL or REJECTED are excluded.
     */
    @GetMapping("/all-approved")
    public ResponseEntity<List<ItemMasterEntity>> getAllApprovedItems() {
        try {
            List<ItemMasterEntity> items = service.getAllApprovedItems();
            return ResponseEntity.ok(items);
        } catch (Exception e) {
            return ResponseEntity.badRequest().build();
        }
